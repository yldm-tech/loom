...
六種類、すべて**同じ描画結果**から作られます。レイアウトのコードが二重に存在することはありません。
...
| `bundle.zip` | 残りの五つ | 一式をまとめて手渡す用。説明書を指す `CLAUDE.md` 付き |
...
残りの五つを一つの書庫にまとめ、中身が `@AGENTS.md` の一行だけの `CLAUDE.md` を添えたものです——Claude Code はこのファイル名を探し、その行が指す先を読み込むので、説明書はマークアップの隣で未読のまま置かれるのではなく、実際に読まれます。写しではなく、指し示すものです。

ボタンが五つあるということは、ファイルを取りこぼす機会が五回あるということで、取りこぼされるのはいつも `AGENTS.md` です。五つのうち、ページらしく見えない唯一のものだからです。受け取った側の手元に残るのはマークアップだけで、どこを変えてよいのか、テーマの取り決めが何なのか、より良いコンポーネントがどこにあるのかは何も分かりません。

zip は手書きで、圧縮なしの STORE です——もう一方の選択肢は依存の追加であり、中身は届いたら一度展開されるだけのテキストだからです。要になっている点が二つあります。サイズと CRC は文字列長ではなく UTF-8 のバイト数で測ります。文面に CJK が入るのは普通のことで、JavaScript の文字列から取った長さは小さく出るからです。そうなった書庫は、その欄を無視するツールでだけ開き、ほかのすべてで失敗します。もう一つ、タイムスタンプは時計ではなく固定値です。同じページを二度書き出せばバイト単位で同一になり、テストがそう言い切れます。
...
あの表の並び順は、それぞれがどれだけ人手を介さず機械から届くかで決まっています——三つは `llms.txt` を公開し、一つは MCP サーバーを動かし、三つは CLI で入ります。そしてその表を書いた loom は、そのどれも公開していませんでした。自分のブロック、テーマ、供給元について知っていることのすべてが、人がタブを開いて押して回らなければ届かない場所にありました。今回足したのは、他所を採点するのに使っていたその surface です。
...
読み取り専用のツールが五つ。コンポーネント供給元の表、あるブロックに効く供給元、13 のブロックとそれぞれのバリアントおよびどの原型が必須とするか、6 つのテーマと Jev が選ぶ際の判断材料、そしてあるテーマの 17 個のトークンとその値です。

答えはどれも、呼び出しが届いた時点で `lib/plan.ts`、`lib/themes.ts`、`lib/sources.ts` から読み出されます。`lib/mcp.ts` はそのどれも書き写していません。ブロック一覧の写しを自前で持つツールは、一覧が変わった後も自信を持って答え続け、向こう側のエージェントにはそれを知る手立てがありません。

JSON-RPC の枠組みは MCP SDK からではなく手で書いてあります。**今回の変更は依存を一つも増やしていない**からです。ツールだけのサーバーがクライアントに負っているのは `initialize`、`tools/list`、`tools/call`、そして `notifications/initialized` に何も返さない自制心だけです——誰も待っていない返事は、以後のすべての応答を別の要求に対応させてしまいます。`scripts/mcp.mjs` はポンプに徹しています。stdin を改行で切り、途中で切れた行の残りを持ち越します。チャンクの境界はメッセージの境界ではなく、本物のクライアントは二つのメッセージが一つのチャンクに収まる速さでハンドシェイクを送ってくるからです。

二つの答えは意図的に、単なる引き当て以上のことをします。loom に無いブロック名には、空の一覧と*その説明*が返ります。推測は決して返しません。実在するのにどの供給元も扱っていないブロックには、その旨をはっきり返します——空の一覧だけでは引き当て失敗のように読めるからです。知らないテーマ名にはフォールバックの配色が返りますが、**フォールバックだと明示されます**。`themeVars()` は知らない名前すべてに `forest` を返し、それは描画としては正しく、説明を付けずにエージェントへ渡すのは誤りです。`terminal` を頼んだつもりのまま、緑のボタンを貼ることになるからです。

### /llms.txt

五つのうち三つが公開しているのと同じ形式で、リクエストのたびに `lib/plan.ts`、`lib/themes.ts`、`lib/sources.ts` から生成されます。動いているデプロイとずれようがありません。テストが実際の表を一つずつ辿るので、`lib/llms.ts` に触れずにブロックやテーマや供給元を足すと、欠落した文書が出回る前にその場で赤くなります。

形は llmstxt.org のとおりです。H1 が一つ、引用が一つ、見出しを含まない本文、そして各行がリンク項目でなければならない H2 の節。最後の規則こそが、ブロック・テーマ・トークン・書き出し・供給元を独立した節ではなく本文に置いている理由です——どれも URL を持たず、形を満たすために `/docs/blocks` を捏造すれば、エージェントを 404 へ送ることになります。他所のエンドポイントはコード片として書いてあるので、エージェントが辿れるリンクはすべて、この origin 自身が答えるものです。

origin はビルド時の定数ではなくリクエストから取ります。同じビルドが localhost でもプレビュー環境でも独自ドメインでも応答するからで、焼き込んだアドレスは誰も試していない環境でだけ現れる種類の誤りです。本当にリンクである節は `POST /api/generate` と `POST /api/edit` の二つで、それぞれのリクエストボディと返ってくるものの形が書いてあります。
...
6 テーマ、それぞれが完結したデザイントークンの一式です。`app/registry.tsx` の色はすべて CSS 変数経由で、**例外は一箇所だけ**、テストがそれを一箇所に留めています——分割ヒーロー内のモック画面にある macOS の信号機ドット 3 つで、これは loom のパレットではなく別の OS のウィンドウ装飾を描いたものです。
...
```bash
npm test          # 362 件、約 1.5 秒、ネットワーク不使用
```
...
ほかにカタログの契約——宣言されるだけでなく実際に検査されるようになりました。`lib/catalog.ts` が定めたスキーマを props が満たさないブロックは、型を鵜呑みにするレンダラーへ渡らずスケルトンのまま場所を保ちます。加えて準備完了の門（空配列は準備済みではなく欠落として扱う）、`asSettled` の完了順、そして JSX 直列化の落とし穴——カスタムプロパティには `as CSSProperties` が要る、グラデーション内のセミコロンは区切りではない、波括弧を含む文字列は包む必要がある、`blk-in` アニメーションクラスと `<style>` は書き出しに入れてはいけない。エージェント向けの面も同じ扱いです。`bundle.zip` は、それを書いた側ではなくテストファイルに別途書いた、わざと素朴な zip リーダーで読み戻します。`/llms.txt` は llmstxt.org の形として解析し、実際のブロック・テーマ・供給元の表と一行ずつ突き合わせます。MCP サーバーはハンドシェイク、公開しているすべてのツール、そして壊れた形で届く一連のメッセージを通して動かします。
...
lib/
  ...
  export-bundle.ts     五つの書き出しと CLAUDE.md を、手書きの zip にまとめる
  llms.ts              /llms.txt、plan.ts・themes.ts・sources.ts から生成
  mcp.ts               5 つの MCP ツールと JSON-RPC の枠組み、純粋かつ同期
...
app/
  ...
  registry.tsx         ブロックの見た目、CSS 変数経由
  llms.txt/            /llms.txt のルート
  api/generate         生成
  api/edit             修正
scripts/
  mcp.mjs              MCP サーバーの stdio ポンプ。プロトコル自体は lib/mcp.ts
  sources.mjs          sources.ts の url とエンドポイントを取り直して確認する
  record-fixture.mjs   実際の実行を fixtures/ へ記録する
  capture-docs.mjs     README のスクリーンショットと録画
  audit-a11y.mjs       起動中のデモモードサーバーに axe-core をかける
```
...
- **MCP サーバーは loom について答えるだけで、loom を動かしはしません。** 五つのツールが読むのはブロック・テーマ・供給元の表です。ページの生成や修正には、いまも `/api/generate` と `/api/edit` を叩くか、エディタを開く必要があります。

## 参加する

[CONTRIBUTING.md](../CONTRIBUTING.md) を参照してください。ブロック・テーマ・UI 言語・コンポーネント供給元・MCP ツールの追加には、それぞれ短い定石があります。
