...
export function buildLlmsTxt(origin: string): string {
  ...
  const body = [
    ...
    "## Generating a page",
    "",
    `- [POST /api/generate](${base}/api/generate): Body ${code('{"prompt": "...", "locale": "en"}')}. Responds ${code("application/x-ndjson")} — one JSON object per line, each tagged with a ${code("type")}: ${code("choice")} for a judgement Jev made and the confidence it made it at, ${code("plan")} for the blocks chosen, ${code("partial")} each time copy lands in another block, ${code("complete")} last. A failure arrives as an ${code("error")} event inside the stream rather than as a non-200; only a missing prompt is a 400. Allow up to 120 seconds. With no keys configured the server replays a recorded session instead of refusing, and the ${code("X-Loom-Mode")} response header reads ${code("demo")} or ${code("live")} so a caller can tell which it got.`,
    "",
    "## Editing a page that already exists",
    "",
    `- [POST /api/edit](${base}/api/edit): Body ${code('{"prompt": "...", "present": ["nav", "hero"], "theme": "forest", "variants": {"hero": "hero_split"}}')}, where ${code("present")} is the page's current blocks and ${code("variants")} its current variant ids. Returns one JSON object: the judged ${code("action")} with its ${code("confidence")}, whichever of ${code("slot")} / ${code("theme")} / ${code("variant")} that action needs, ${code("blockedBy")} when the request named no target confidently enough to act on, and ${code("speculative")} holding every branch Jev scored. One round trip, and no copy is regenerated.`,
    "",
    "## Optional",
    "",
