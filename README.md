...
支持 `zh` / `en` / `ja` / `zh-Hant`。字数约束是按中文写的，其它语言会附一条换算提示。
...
**测试写完做了变异验证**：把阈值 5 改成 4、把首屏条件反过来、让空数组算就绪、去掉类型断言、不再剔除 style 块——五个变异全部被抓到。第一次写就全过的测试是可疑的。
...
- **界面本身只有中文。** 生成的站点支持四种语言，但编辑器的按钮和提示没做 i18n。
