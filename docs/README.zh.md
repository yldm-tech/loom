...
6 套主题，每套是一组完整的设计 token。`app/registry.tsx` 里的颜色全部走 CSS 变量，**只有一处例外**，并且有测试把它钉在一处：分栏 hero 那张模拟截图上的三个 macOS 红绿灯圆点，它画的是另一个操作系统的窗口装饰，不是 loom 的配色：
...
另外测了目录契约——它现在是被执行的而不只是被声明的：props 不满足 `lib/catalog.ts` 为它写下的 schema 的区块，会以骨架占位，而不是进到一个只做类型断言的渲染器里；还测了就绪度门控（空数组算缺失而不是就绪）、`asSettled` 的完成顺序、以及 JSX 序列化的几个坑：CSS 自定义属性要带 `as CSSProperties`、渐变里的分号不能当分隔符、含花括号的文本要包起来、`blk-in` 动画类和 `<style>` 块不能带进导出。面向 agent 的那几个面也一样待遇：`bundle.zip` 由测试文件里另写的一个故意慢吞吞的 zip 读取器读回来，而不是用生成它的那个写入器；`/llms.txt` 按 llmstxt.org 的形状解析，并逐行和真实的区块、主题、来源表对账；MCP 服务则被完整走一遍握手、它对外声明的每个工具，以及一串本身就是坏的消息。
